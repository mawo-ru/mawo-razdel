...
class SynTagRusPatterns:
    ...
    ABBREVIATIONS = {
        # Географические
        "г", "гг", "г-н", "г-жа",  # Год, годы, господин, госпожа
        "ул", "пр", "пл", "пер", "просп", "наб",  # Улица, проспект, площадь...
        "д", "дом", "корп", "стр", "кв",  # Дом, корпус, строение, квартира
        "обл", "р-н", "п", "с", "дер", "пос",  # Область, район, посёлок...
        # Научные степени и звания
        "акад", "проф", "доц", "к", "канд", "докт",  # Академик, профессор...
        "м", "н", "мл", "ст",  # Младший, старший научный сотрудник
        # Титулы
        "им", "ген", "полк", "подп", "лейт", "кап",  # Имени, генерал...
        # Временные
        "в", "вв", "р", "руб", "коп",  # Век, рубль, копейка
        "ч", "час", "мин", "сек",  # Час, минута, секунда
        # Общие сокращения
        "т", "тт", "пп", "рис", "илл", "табл", # Том, пункт, рисунок...
        "см", "ср", "напр", "в т.ч", "и т.д", "и т.п", "и др",  # Смотри, сравни...
        "др", "проч", "прим", "примеч",  # Другое, прочее, примечание
        # Измерения
        "кг", "мг", "ц", "л", # Килограмм, грамм...
        "мм", "км", "га",  # Метр, сантиметр...
        "млн", "млрд", "тыс", "трлн",  # Миллион, миллиард...
        # Организационные
        "о-во", "о-ва", "о-ние", "о-ния",  # Общество, общества...
        "зам", "пом", "зав", "нач",  # Министр, заместитель...
        # Прочие
        "etc", "et al", "ibid", "op cit",  # Латинские
        "англ", "нем", "франц", "итал", "исп",  # Языки
    }

    # Почетные звания и должности (часто перед ФИО)
    TITLES = {
        "президент", "премьер", "министр", "губернатор", "мэр",
        "директор", "председатель", "генеральный",
        "академик", "профессор", "доктор",
        "господин", "госпожа", "товарищ",
    }

    # Слова, после которых часто идет прямая речь
    SPEECH_VERBS = {
        "сказал", "сказала", "сказали", "говорил", "говорила",
        "ответил", "ответила", "спросил", "спросила",
        "заявил", "заявила", "отметил", "отметила",
        "подчеркнул", "подчеркнула", "добавил", "добавила",
        "пояснил", "пояснила", "уточнил", "уточнила",
    }
    ...
    def _compile_patterns(self) -> None:
        ...
        self.rules: list[SegmentationRule] = [
            # Priority 1: NEVER break on these
            SegmentationRule(
                name="abbreviation_dot",
                pattern=re.compile(
                    r"(?<=" + "|".join(re.escape(abbr) for abbr in self.ABBREVIATIONS) + r")\."
                ),
                is_boundary=False,
                priority=100,
                description="Dot after abbreviation (not a sentence boundary)",
            ),
            # Priority 2: Initials (А. С. Пушкин)
            SegmentationRule(
                name="initials",
                pattern=re.compile(r"(?<=[А-ЯЁ])\.\s+(?=[А-ЯЁ]\.|\b[А-ЯЁ][а-яё]+\b)"),
                is_boundary=False,
                priority=95,
                description="Initials (e.g., А. С. Пушкин)",
            ),
            # Priority 3: Decimal numbers (3.14, 2.5)
            SegmentationRule(
                name="decimal_numbers",
                pattern=re.compile(r"(?<=\d)\.(?=\d)"),
                is_boundary=False,
                priority=90,
                description="Decimal point in numbers",
            ),
            # Priority 4: Ellipsis (...) - not a sentence boundary
            SegmentationRule(
                name="ellipsis",
                pattern=re.compile(r"\.{2,}"),
                is_boundary=False,
                priority=85,
                description="Ellipsis (multiple dots)",
            ),
            # Priority 5: Direct speech continuation
            # "Здравствуйте, - сказал он. - Как дела?"
            SegmentationRule(
                name="speech_continuation",
                pattern=re.compile(
                    r'(?<=[.!?])\s*[-—–]\s*(?:'
                    + "|".join(self.SPEECH_VERBS)
                    + r')\s+\w+\.\s*[-—–]'
                ),
                is_boundary=False,
                priority=80,
                description="Direct speech with interruption",
            ),
